...
In this work, we mainly focus on the privacy of garbling an AND gate in the Free XOR \acrlong{gs}. We also consider NOT and XOR gates as important, however, we do not prove their security explicitly since NOT gates are trivially usable by inverting the active wire label of the next gate in the circuit that the NOT gate is connected to. This means that a NOT gate does not need to be explicitly garbled, and we can just flip the active bit-value of that wire when garbling the following gate. The \cevaluator only receives the garbling of the flipped bit-value and never learns the corresponding unflipped value, preserving the secrecy of circuit's global offset, $\Delta$.

A similar argument can be made for the usage of an XOR gate, since it is not explicitly garbled, and the \cgarbler just indicates the presence of an XOR gate and the \cevaluator simply XORs the two input wire labels to obtain the output label. The \cevaluator never learns both possible wire labels for any wire, and privacy remains intact without needing to prove anything about a garbled table. 
