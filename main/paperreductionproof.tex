...
\begin{definition}\label{def:linrkkdmsecurity}
    An encryption scheme is said to be \acrshort{linrkkdm} secure if any adversary $\adv$ interacting with $G^{b}_{\mathsf{LIN-RK-KDM}}$ (Figure \ref{fig:linrkkdm-game}) in the real and ideal world cannot distinguish between them with more than negligible advantage, i.e.,
...
Since we now have a base assumption to build from, we now try to reduce the security of the real and simulated garbling games (Figures \ref{fig:simfreexor-real-game} and \ref{fig:simfreexor-ideal-game}) to the \acrshort{linrkkdm} assumption. 
...
\[\begin{aligned}
    ...
    \text{and }&G^{1}_\mathsf{Sim\text{-}FreeXOR} &&\codeequiv \rdv\to G^{1}_{\mathsf{LIN-RK-KDM}} &&&\text{(Lemma \ref{lemma:idealcodeeq})}
\end{aligned}\]
